    ...
    try{
      ...
      if(status >=200 && status < 300){
        ...
        try{
          ...
          return { success: true, statusCode: status, body: json, attempts: attempt };
        }catch(e){
          ...
          return { success: false, error: 'API_INVALID_JSON', message: 'Invalid JSON response', retriable: false, statusCode: status, rawBody, headers: hdrs, attempts: attempt };
        }
      }
      ...
      if(isTransientStatus(status)){
        ...
        return { success: false, error: 'API_INTERNAL_ERROR', message: `HTTP ${status}`, retriable: true, statusCode: status, attempts: attempt };
      }
      ...
      return { success: false, error: errCode, message: `HTTP ${status}`, retriable: false, statusCode: status, attempts: attempt };
      ...
      return { success: false, error: classification.code, message: err.message || String(err), retriable: classification.retriable, severity: classification.severity, attempts: attempt };
    }
  }
}
...
async function callMiner(input){
  ...
  if(queryParams){
    Object.keys(queryParams).forEach(k=>{
      if(queryParams[k] !== undefined && queryParams[k] !== null){
        urlObj.searchParams.append(k, String(queryParams[k]));
      }
    });
  }
  ...
  // Attach API key headers when available (Authorization set as Bearer and X-API-Key)
  if(API_KEY){
    try{
...
export { callMiner, API_CONFIG, endpointIsApproved };